        ...
        for (i, line) in chars.iter().enumerate() {
            for (j, c) in line.iter().enumerate() {
                ...
                for &(di, dj) in &directions {
                    let mut y = i as i32+di;
                    let mut x = j as i32+dj;
                    while 0<=y && y<chars.len() as i32 && 0<=x && x<line.len() as i32 {
                        let yu = y as usize;
                        let xu = x as usize;
                        if chars[yu][xu] != '.' {
                            adj.push(chars[yu][xu]);
                            break;
                        }
                        y += di;
                        x += dj;
                ...
                if *c == '#' && adj.iter().filter(|&&adj_c| adj_c == '#').count() >= 5 {
                    next_chars[i][j] = 'L';
                }
            }
        }
