...
drab purple bags contain 1 shiny indigo bag, 4 striped yellow bags.";
    ...
    let mut children_parents: HashMap<&str, Vec<&str>> = HashMap::new();
    ...
    for line in lines {
        ...
        for child in children.split(", ") {
            ...
            // println!("{} {}", count, name);
            children_parents.entry(&name).or_insert(Vec::new()).push(parent);
        }
        // println!("{:?}", parent);
    }
    let mut stack: Vec<&str> = children_parents.get("shiny gold").unwrap().to_vec();
    let mut ancestors: HashSet<&str> = HashSet::new();
    while !stack.is_empty() {
        // println!("{:?}", stack);
        let parent = stack.pop().unwrap();
        ancestors.insert(parent);
        match children_parents.get(&parent) {
            Some(parents) => stack.extend_from_slice(&parents.to_vec()[..]),
            None => (),
        }
    }
    println!("{}", ancestors.iter().count())
}
// ans 265