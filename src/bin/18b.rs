fn main() {
    let input = "(9 * 3 + 4) * (7 + (2 + 3 + 6 * 2) + 3) * 4 * 5
...
3 + (4 + 8 + 3 + 4 + 7 + 6) + 4 + 3 * 4 + ((5 * 6) + 2 * 5 * 2 + 8 * 3)";
    ...
    for line in input.lines() {
        ...
        for c in line.chars() {
            match c {
                '0'..='9' => {
                    ...
                    let currnum = c.to_digit(10).map(u64::from).unwrap();
                    if *lastop == '+' {
                        ops.pop();
                        let lastnum = numbers.pop().unwrap();
                        numbers.push(lastnum + currnum);
                    } else {
                ...
                ')' => {
                    ...
                    while let Some(c) = ops.pop() {
                        ...
                        let currnum = numbers.pop().unwrap();
                        let lastnum = numbers.pop().unwrap();
                        let result = match c {
                            '+' => lastnum + currnum,
                            '*' => lastnum * currnum,
        ...
        // evaluate all the remaining operators, there should only be at most 1 '+' at the last operator
        // println!("{:?}", numbers);
        // println!("{:?}", ops);
        while let Some(c) = ops.pop() {
            let currnum = numbers.pop().unwrap();
