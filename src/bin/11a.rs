fn main() {
    let input = "LLLLLLLLLLLLLLLLLLLLLL.LLLLLLLL.LLLLLLLLLLLLLL.LLLLL.LLLLL.LLLL.LLLL.LLLLLLLLLL.LLLLLL.LLLLL
...
LLLLLLLLLL.LLLLL.LLLLLLL.LLLLLL.LL.LLLLLLLLLLL.LLLLL.LLLLL.LLLLLLLLL.LLLL.LLLLL.LLL.LLLLLLLL";
    let mut chars: Vec<Vec<char>> = input.lines().map(|x| x.chars().collect()).collect();
    let mut next_chars: Vec<Vec<char>>;
    let mut count = 0;
    loop {
        next_chars = chars.clone();
        for (i, line) in chars.iter().enumerate() {
            for (j, c) in line.iter().enumerate() {
                let mut adj: Vec<char> = vec![];
                // get adjacent cells, src: https://stackoverflow.com/a/2035539/4858751
                for di in if i > 0 {-1} else {0} ..= if i < chars.len() - 1 {1} else {0} { // if else as ternary operator
                    for dj in if j > 0 {-1} else {0} ..= if j < line.len() - 1 {1} else {0} {
                        if di != 0 || dj != 0 {
                            adj.push(chars[(i as i32+di) as usize][(j as i32+dj) as usize]);
                        }
                    }
                }
                // println!("{},{} {:?}", i,j, adj);
                if *c == 'L' && adj.iter().all(|&adj_c| adj_c != '#') {
                    next_chars[i][j] = '#';
                }
                if *c == '#' && adj.iter().filter(|&&adj_c| adj_c == '#').count() >= 4 {
                    next_chars[i][j] = 'L';
                }
            }
        }
        ...
        if next_chars == chars {
            ...
            println!("{}", count);
            println!("{}", next_chars.iter().map(|line| line.iter().collect::<String>()).collect::<Vec<String>>().join("\n"));
            println!("occupied: {}\n", next_chars.iter().fold(0, |a, line| line.iter().filter(|&&c| c == '#').count() + a));
            chars = next_chars;
        }
    }
}
