...
bchbc zphlmnb frnz fzkxc xbdh zpkl btpdk csdbg zbhmgm nrbl qpzcmf rtjk mxpn cqzqvj hhgqp zpcscz bgvng mpbsn shptr hzfd vdm fkszg lvrp lzxgf bltzkxx dpmcrh nzhtc kbzmtz vxkkgd hflzrt kqth gtgkt ggqsr rghrrt bclc mfqvks dtvsk bltrbvz vgh tprdg zvtvzc txs pxldx bbhvzz sncl mhvtp kjfnqg hcnmld krh hzr blvlch qfkgx spcqmzfg qtxgzpx bqrpdxt krsl rbxnc dzqlq rpf pflk fxsrc zlcbnx bxcgf zdznn jgblz (contains fish)";
    ...
    for line in lines {
        ...
        // allingredients = allingredients.union(&ingredients).map(|&s| s).collect();
        for ingredient in &ingredients {
            *ingredients_count.entry(ingredient).or_insert(0) += 1;
        }
        // println!("{:?}", ingredients);
        if split.len() > 1 {
            let allergens = &split[1][..split[1].len()-1];
            // println!("allergens {:?}", allergens);
            for allergen in allergens.split(", ") {
                // println!("  {}", allergen);
                if allergen_ingredients.contains_key(allergen) {
                    let existing_ingredients = allergen_ingredients.get_mut(allergen).unwrap();
                    *existing_ingredients = existing_ingredients.intersection(&ingredients).map(|&s| s).collect();
                } else {
                    allergen_ingredients.insert(allergen, ingredients.clone());
                }
            }
        }
    }
    println!("{:?}", allergen_ingredients);
    println!("{:?}", ingredients_count);
    let mut ingredient_allergen: HashMap<&str, &str> = HashMap::new();
    let mut changed = true;
    while changed {
        changed = false;
        for (allergen, ingredients) in allergen_ingredients.iter_mut() {
            println!("{} {:?}", allergen, ingredients);
            *ingredients = ingredients.difference(&ingredient_allergen.keys().map(|&s| s).collect()).map(|&s| s).collect();
            println!("{} {:?}", allergen, ingredients);
            if ingredients.len() == 1 {
                ingredient_allergen.insert(ingredients.iter().next().unwrap(), allergen);
                changed = true;
            }
        }
        println!("allergen_ingredients {:?}", allergen_ingredients);
        println!("ingredient_allergen {:?}", ingredient_allergen);
    }

    println!("ingredient_allergen count {}", ingredient_allergen.len());
    println!("ingredient_count {:?}", &ingredients_count);
    println!("ingredient_count filtered {:?}", &ingredients_count.iter().filter(|&(k, _)| !ingredient_allergen.contains_key(k)).map(|(&k, &v)| (k,v)).collect::<HashMap<&str, usize>>());
    println!("ans {}", ingredients_count.iter().filter(|&(k, _)| !ingredient_allergen.contains_key(k)).map(|(_,v)| v).sum::<usize>())
}
// ans 1930