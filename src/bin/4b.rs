...
fn main() {
    let input = "eyr:2029 byr:1931 hcl:z cid:128
...
cid:209 hcl:#efcc98 byr:2001";
    let batches: Vec<&str> = input.split("\n\n")
        .filter(|batch| {
            let batch_key_iter: Vec<&str> = batch
                .split_whitespace()
                .map(|kv| {
                    ...
                    let m = match key {
                        "byr" => 1920 <= value_int && value_int <= 2002,
                        "iyr" => 2010 <= value_int && value_int <= 2020,
                        "eyr" => 2020 <= value_int && value_int <= 2030,
                        "hgt" => {
                            let unit = &value[value.len()-2..];
                            let height: u8 = value[..value.len()-2].parse().unwrap_or(0);
                            match unit {
                        ...
                        "ecl" => "amb blu brn gry grn hzl oth".split(" ").any(|v| value == v),
            ...
            "byr iyr eyr hgt hcl ecl pid"
                .split(" ")
                .all(|key| batch_key_iter.contains(&key))
        })
        .collect();
    println!("{}", batches.len());
}
