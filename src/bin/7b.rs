...
fn main() {
    let input = "shiny aqua bags contain 1 dark white bag.
...
drab purple bags contain 1 shiny indigo bag, 4 striped yellow bags.";
    ...
    let mut parents_children: HashMap<&str, HashMap<&str, i32>> = HashMap::new();
    ...
    for line in lines {
        ...
        for child in children.split(", ") {
            let caps = re.captures(child).unwrap();
            let count: i32 = caps["count"].parse().unwrap_or(0);
            let name = caps.name("name").unwrap().as_str();
            parents_children.entry(parent).or_insert(HashMap::new()).insert(name, count);
        }
    }
    // println!("{:?}", parents_children);
    // original version using vector as call stack
    // let mut stack: Vec<(&str, i32)> = parents_children.get("shiny gold").unwrap().iter().map(|(k,v)| (*k,*v)).collect();
    // println!("{:?}", stack);
    // let mut sum = 0;
    // while !stack.is_empty() {
    //     println!("{:?}", stack);
    //     let (child, count) = stack.pop().unwrap();
    //     sum += count;
    //     match parents_children.get(child) {
    //         Some(children) => {
    //             stack.extend_from_slice(&children.iter().map(|(name, c)| (*name, c * count)).collect::<Vec<(&str, i32)>>()[..])
    //         },
    //         None => (),
    //     }
    // }
    // improved version using hashmap as "call stack"
    let mut stack: HashMap<&str, i32> = parents_children.get("shiny gold").clone().unwrap().iter().map(|(k,v)| (*k,*v)).collect();
    let mut sum = 0;
    while !stack.is_empty() {
        // println!("{:?}", stack);
        let k = stack.keys().next().cloned().unwrap();
        let (child, count) = stack.remove_entry(k).unwrap();
        sum += count;
        match parents_children.get(child) {
            Some(children) => {
                // instead of adding all into a stack, just update existing children bags count
                for (name, c) in children {
                    *stack.entry(name).or_insert(0) += count * c
                }
            },
            None => (),
        }
