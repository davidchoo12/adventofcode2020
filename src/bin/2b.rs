...
fn main() {
    let input = "6-10 p: ctpppjmdpppppp
...
8-11 t: tttttttcttm";
    ...
    let re = Regex::new(r"(?P<min>\d+)-(?P<max>\d+) (?P<char>\w): (?P<pw>\w+)").unwrap();
    let mut valid = 0;
    for line in lines {
        let caps = re.captures(line).unwrap();
        let min = caps["min"].parse::<usize>().unwrap() - 1;
        let max = caps["max"].parse::<usize>().unwrap() - 1;
        let c: char = caps["char"].parse().unwrap();
        let first_match = caps["pw"].chars().nth(min).unwrap() == c;
        let last_match = caps["pw"].chars().nth(max).unwrap() == c;
